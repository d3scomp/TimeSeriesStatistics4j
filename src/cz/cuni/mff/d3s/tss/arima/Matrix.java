...
public class Matrix {
     ...
     */
	Matrix(Layout layout, final double[]... matrixData) {
        this.layout = layout;
	...
	public Matrix pushColumn(Vector newData) {
		...
		return new Matrix(Layout.BY_COLUMN, newMatrix);
	}
	...
	/**
	 * A class that allows one to start with an identity matrix, then set specific
	 * elements before creating an immutable matrix.
	 *
	...
	static final class IdentityBuilder {
		...
		/**
		 * Set the matrix at the given coordinates to the provided value and return the
		 * builder.
		 *
		 * @param i
	...
	/**
	 * A class that allows one to start with a zero matrix, then set specific
	 * elements before creating an immutable matrix.
	 *
	 * @author Jacob Rachiele
	 */
	static final class ZeroBuilder {
		...
		static void validateRow(int i, int m) {
			...
			if (i < 0) {
				throw new IllegalArgumentException(
						"The row index must be greater than or equal to zero, " + "but was " + i);
			}
		}
		...
		static void validateColumn(int i, int n) {
			...
			if (i < 0) {
				throw new IllegalArgumentException(
						"The column index must be greater than or equal to zero, " + "but was " + i);
			}
		}
		...
		static void validateSize(int n, Vector vector) {
			if (vector.size() != n) {
				throw new IllegalArgumentException(
						"The vector must have " + n + " elements, " + "but had " + vector.size());
			}
		}
	}
    ...
    static Matrix create(Layout layout, final double[]... matrixData) {
        return new Matrix(layout, matrixData);
    }
     ...
     */
    static MatrixOneD create(final int nrow, final int ncol, final double... data) {
     ...
     */
    static MatrixOneD create(final double[]... matrixData) {
        return new MatrixOneD(MatrixOneD.Layout.BY_ROW, matrixData);
    }
