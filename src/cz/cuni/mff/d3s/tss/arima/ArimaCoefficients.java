...
public class ArimaCoefficients {
	...
	/**
	 * Create a structure holding the coefficients, the degrees of differencing, and
	 * the mean of a seasonal ARIMA model.
	 *
	 * @param arCoeffs
	 *            the non-seasonal autoregressive coefficients.
	 * @param maCoeffs
	 *            the non-seasonal moving-average coefficients.
	 * @param sarCoeffs
	 *            the seasonal autoregressive coefficients.
	 * @param smaCoeffs
	 *            the seasonal moving-average coefficients.
	 * @param d
	 *            the non-seasonal degree of differencing.
	 * @param D
	 *            the seasonal degree of differencing.
	 * @param mean
	 *            the process mean.
	 */
	ArimaCoefficients(double[] arCoeffs, double[] maCoeffs, double[] sarCoeffs, double[] smaCoeffs, int d, int D,
			double mean, double drift, int seasonalFrequency) {
	...
	// Expand the autoregressive coefficients by combining the non-seasonal and
	// seasonal coefficients into a single
	// array, which takes advantage of the fact that a seasonal AR model is a
	// special case of a non-seasonal
	...
	static double[] expandArCoefficients(final double[] arCoeffs, final double[] sarCoeffs,
		...
		// Note that we take into account the interaction between the seasonal and
		// non-seasonal coefficients,
		// which arises because the model's ar and seasonal ar polynomials are
		// multiplied together.
	...
	// Expand the moving average coefficients by combining the non-seasonal and
	// seasonal coefficients into a single
	// array, which takes advantage of the fact that a seasonal MA model is a
	// special case of a non-seasonal
	...
	static double[] expandMaCoefficients(final double[] maCoeffs, final double[] smaCoeffs,
		...
		// Note that we take into account the interaction between the seasonal and
		// non-seasonal coefficients,
		// which arises because the model's ma and seasonal ma polynomials are
		// multiplied together.
		// In contrast to the ar polynomial, the ma and seasonal ma product maintains a
		// positive sign.
	...
	/**
	 * Get the model intercept term. Note that this is <i>not</i> the model mean, as
	 * in R, but the actual intercept. The intercept is equal to &mu; &times; (1 -
	 * sum(AR)), where &mu; is the model mean and AR is a vector containing the
	 * non-seasonal and seasonal autoregressive coefficients.
	 *
	...
	/**
	 * Computes and returns the order of the ARIMA model corresponding to the model
	 * coefficients.
	 *
	 * @return the order of the ARIMA model corresponding to the model coefficients.
	 */
	ArimaOrder extractModelOrder() {
		ArimaOrder.Constant constant = (Math.abs(this.mean) > EPSILON) ? ArimaOrder.Constant.INCLUDE
				: ArimaOrder.Constant.EXCLUDE;
		ArimaOrder.Drift drift = (Math.abs(this.drift) > EPSILON) ? ArimaOrder.Drift.INCLUDE : ArimaOrder.Drift.EXCLUDE;
		return new ArimaOrder(arCoeffs.length, d, maCoeffs.length, sarCoeffs.length, D, smaCoeffs.length, constant,
				drift);
	}
	...
	public static class Builder {
		...
		/*
		 * public MultipleLinearRegressionBuilder setDrift(double drift) { this.drift =
		 * drift; return this; }
		 */
